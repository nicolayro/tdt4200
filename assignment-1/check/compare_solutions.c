...
int main ( int argc, char **argv )
{
    ...
    int64_t solution_size = atoi ( argv[1] );

    double *solution_1 = malloc ( solution_size * sizeof(double) );
    double *solution_2 = malloc ( solution_size * sizeof(double) );

    load_solution ( argv[2], solution_1, solution_size );
    load_solution ( argv[3], solution_2, solution_size );

    uint64_t errors = compare_solutions ( solution_1, solution_2, solution_size );
    fprintf ( stderr, "There are %llu differences between the solution and the reference solution\n", errors );
